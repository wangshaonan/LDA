...
def hdplda_learning(hdplda, iteration, voca,corpuswrite1, corpuswrite2):
    for i in range(iteration):
        hdplda.inference()
        print "-%d K=%d p=%f" % (i + 1, len(hdplda.topics), hdplda.perplexity())
    output_word_topic_dist(hdplda, voca, corpuswrite1)
    output_topic_doc_dist(hdplda, corpuswrite2)
    return hdplda

def output_word_topic_dist(hdplda, voca, corpuswrite1):
    phi = hdplda.worddist()
    for k in xrange(len(hdplda.topics)):
...
def output_topic_doc_dist(hdplda, corpuswrite2):
    theta = []
    for j in range(len(hdplda.x_ji)):
        p_k = numpy.zeros(max(hdplda.topics)+1)
        for t in hdplda.tables[j]:
            k = hdplda.k_jt[j][t]
            p_k[k] += hdplda.n_jt[j][t]
        theta.append(p_k)
